 ...
 *
 * Reusable subcategory form for Create and Edit modes.
...
import SubCategoryFormFields from "./SubCategoryFormFields.js";
import SubCategoryFormActions from "./SubCategoryFormActions.js";
...
export default function SubCategoryForm({
  ...
  useEffect(() => {
    if (initialValues && Object.keys(initialValues).length > 0) {
      setValues({
        ...
        categoryId: initialValues.categoryId || initialValues.category?.id || initialValues.category?._id || "",
      });
    }
  }, [initialValues]);
  ...
  const validateForm = () => {
    ...
    if (!values.categoryId || values.categoryId.trim().length === 0) {
      newErrors.categoryId = "La catégorie parente est requise";
    }
  ...
  return (
    <FormContainer onSubmit={handleSubmit}>
      <FormSection>
        <SectionTitle>
          {mode === "create" ? "Nouvelle sous-catégorie" : "Modifier la sous-catégorie"}
        </SectionTitle>
        ...
        <SubCategoryFormActions mode={mode} isLoading={isLoading} />
      </FormSection>
    </FormContainer>
  );
}
