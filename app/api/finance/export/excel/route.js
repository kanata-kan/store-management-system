/**
 * Finance Dashboard Excel Export API Route
 *
 * GET /api/finance/export/excel?startDate=&endDate=
 *
 * Generates an Excel report of financial data for a date range.
...
// Force dynamic rendering for this route (always generate fresh Excel)
export const dynamic = "force-dynamic";

/**
 * GET /api/finance/export/excel
 * Generate Excel report of financial data
 * Authorization: Manager only
 */
export async function GET(request) {
  try {
    ...
    const excelBuffer = await FinanceExportService.generateExcel(exportData);
    ...
    const endDateStr =
      exportData.endDate && exportData.endDate !== "-"
        ...
        : new Date().toISOString().split("T")[0];
    const filename = `rapport-financier_${startDateStr}_${endDateStr}.xlsx`;
