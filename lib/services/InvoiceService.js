...
class InvoiceService {
  ...
  static async generatePDF(invoiceId, user) {
    ...
    invoice.items.forEach((item, index) => {
      ...
      // Warranty info - Improved display
      ...
      if (item.warranty?.hasWarranty) {
        ...
        if (item.warranty.expirationDate) {
          ...
          const startDate = item.warranty.startDate ? new Date(item.warranty.startDate) : invoice.createdAt ? new Date(invoice.createdAt) : new Date();
          warrantyText = `${durationMonths || ""} mois${durationMonths ? "" : ""}\nExpire: ${expDate.toLocaleDateString("fr-FR")}`;
        } else if (durationMonths) {
          warrantyText = `${durationMonths} mois`;
        } else {
          warrantyText = "Oui";
        }
      }
    ...
    // Warranty section (if any item has warranty) - Add after totals
    const hasAnyWarranty = invoice.items.some((item) => item.warranty?.hasWarranty);
    if (hasAnyWarranty) {
      doc.moveDown(2);
      const warrantySectionY = doc.y;
      addText("INFORMATIONS DE GARANTIE", {
        size: 12,
        bold: true,
        y: warrantySectionY,
      });
      ...
      invoice.items.forEach((item, index) => {
        if (item.warranty?.hasWarranty) {
          ...
          const detailsY = itemY + 12;
          let warrantyDetails = [];
          
          if (item.warranty.durationMonths) {
            warrantyDetails.push(`Durée: ${item.warranty.durationMonths} mois`);
          }
          
          if (item.warranty.startDate) {
            const startDate = new Date(item.warranty.startDate);
            warrantyDetails.push(`Début: ${startDate.toLocaleDateString("fr-FR")}`);
          ...
          if (item.warranty.expirationDate) {
            const expDate = new Date(item.warranty.expirationDate);
            warrantyDetails.push(`Expiration: ${expDate.toLocaleDateString("fr-FR")}`);
          }
          
          if (warrantyDetails.length > 0) {
            addText(warrantyDetails.join(" | "), {
              y: detailsY,
              size: 9,
              color: "#333333",
            });
