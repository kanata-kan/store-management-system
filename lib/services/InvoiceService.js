 ...
 * 
 * Accountant-friendly numbering system with document type prefixes.
 * 
 * Current format: INV-YYYYMMDD-XXXX (for all documents)
 * 
 * Future-ready: Can be extended to use different prefixes:
 * - FACTURE → FAC-YYYYMMDD-XXXX
 * - BON DE VENTE → BV-YYYYMMDD-XXXX
 * 
 * @param {Date} saleDate - Sale date
 * @param {string} documentType - Document type: "RECEIPT" or "INVOICE"
 * @param {string} documentTitle - Document title: "FACTURE" | "FACTURE SANS TVA" | "BON DE VENTE"
 * @returns {Promise<string>} Document number
 */
async function generateInvoiceNumber(saleDate, documentType, documentTitle) {
  // Date format: YYYYMMDD
  const dateStr = `${saleDate.getFullYear()}${String(
    saleDate.getMonth() + 1
  ).padStart(2, "0")}${String(saleDate.getDate()).padStart(2, "0")}`;

  // CURRENT IMPLEMENTATION: Use "INV" prefix for all documents
  // This is safe and accountant-friendly (sequential numbering)
  const prefix = "INV";
  
  // FUTURE-READY: Uncomment below to enable prefixed numbering by document type
  // This can be activated later when client requests it
  /*
  let prefix;
  ...
  */
  ...
  // Find the last invoice number for today with same prefix
  ...
  // Format: PREFIX-YYYYMMDD-XXXX (4 digits)
...
class InvoiceService {
   ...
   * @param {string} data.documentType - Document type: "RECEIPT" or "INVOICE"
   ...
   * @param {number} data.sellingPriceHT - Selling price HT per unit
   * @param {number} data.tvaRate - TVA rate (may be 0)
  ...
  static async createInvoiceFromSale(data) {
    const {
      saleId,
      documentType, // NEW: "RECEIPT" or "INVOICE"
      customer,
      productId,
      quantity,
      sellingPriceHT, // HT price (NOT TTC)
      tvaRate = 0, // May be 0
      cashierId,
      saleDate = new Date(),
    } = data;
    ...
    if (!saleId || !documentType || !customer || !productId || !quantity || !sellingPriceHT || !cashierId) {
      throw createError(
        "Champs requis manquants: saleId, documentType, customer, productId, quantity, sellingPriceHT, cashierId",
        "VALIDATION_ERROR",
        400
      );
    }
    ...
    if (!["RECEIPT", "INVOICE"].includes(documentType)) {
      throw createError(
        "documentType must be 'RECEIPT' or 'INVOICE'",
        "VALIDATION_ERROR",
        400
      );
    }
    ...
    // Calculate TVA amounts (may be 0)
    const tvaAmountPerUnit = sellingPriceHT * tvaRate;
    const totalTvaAmountPerItem = tvaAmountPerUnit * quantity;
    const sellingPriceTTCPerUnit = sellingPriceHT * (1 + tvaRate);
    const totalPriceHT = sellingPriceHT * quantity;
    const totalPriceTTC = sellingPriceTTCPerUnit * quantity;

    // Build invoice item with TVA breakdown
    const invoiceItem = {
      productSnapshot,
      quantity,
      unitPriceHT: sellingPriceHT, // HT per unit
      tvaRate: tvaRate, // TVA rate snapshot (may be 0)
      tvaAmount: totalTvaAmountPerItem, // TVA for this item (may be 0)
      totalPriceHT: totalPriceHT, // Total HT
      totalPriceTTC: totalPriceTTC, // Total TTC
      warranty,
      // Backward compatibility (deprecated)
      unitPrice: sellingPriceHT,
      totalPrice: totalPriceHT,
    };

    // Calculate invoice totals
    const subtotal = totalPriceHT; // HT total
    const tvaAmount = totalTvaAmountPerItem; // Total TVA (may be 0)
    const totalAmount = totalPriceTTC; // TTC total (equals HT if no TVA)

    // Determine document title (LEGAL COMPLIANCE)
    let documentTitle;
    if (tvaRate > 0) {
      documentTitle = "FACTURE"; // Has TVA
    } else if (documentType === "INVOICE") {
      documentTitle = "FACTURE SANS TVA"; // Invoice but no TVA
    } else {
      documentTitle = "BON DE VENTE"; // Receipt
    }

    // Generate invoice/document number (with document type support)
    const invoiceNumber = await generateInvoiceNumber(saleDate, documentType, documentTitle);
    ...
    const invoice = new Invoice({
      ...
      documentTitle, // "FACTURE" | "FACTURE SANS TVA" | "BON DE VENTE"
      customer: {
      ...
      items: [invoiceItem],
      subtotal, // HT
      tvaAmount, // Total TVA (may be 0)
      totalAmount, // TTC (equals HT if no TVA)
      cashier: cashierId,
      status: "active",
    });
