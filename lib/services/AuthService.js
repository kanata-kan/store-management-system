...
class AuthService {
  ...
  static async login(email, password) {
    ...
    if (!email || !password) {
      throw createError("Email et mot de passe sont requis", "VALIDATION_ERROR");
    }
    ...
    if (!user) {
      throw createError("Email ou mot de passe incorrect", "INVALID_CREDENTIALS", 401);
    }
  ...
  static async getUserFromSession(token) {
    ...
    if (!token) {
      throw createError("Token de session requis", "UNAUTHORIZED", 401);
    }
    ...
    try {
      ...
      if (
        ...
        throw createError("Session invalide ou expirée", "SESSION_EXPIRED", 401);
      }
      throw error;
    }
  }
