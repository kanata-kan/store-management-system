...
<body>
  <div class="invoice-container">
    ...
    <div class="store-info">
      <div class="section-title">{{storeName}}</div>
      <p>📍 {{storeAddress}}</p>
      <p>📞 {{storePhone}}</p>
      {{storeEmailLine}}
    </div>
    ...
    <div class="invoice-footer">
      <p>{{footerText}}</p>
      <p style="margin-top: 10px; font-size: 9px; font-style: italic;">{{warrantyNotice}}</p>
      <p style="margin-top: 8px; color: #9ca3af;">Facture générée le {{generatedDate}}</p>
    </div>
  </div>
</body>
</html>`;
...
export function renderInvoiceHTML(invoice, storeSettings = null) {
  // Store information (from settings or defaults)
  const storeName = storeSettings?.storeName || "Système de Gestion de Magasin";
  const storeAddress = storeSettings?.address || "Adresse du magasin";
  const storePhone = storeSettings?.phoneLandline || storeSettings?.phoneWhatsApp || "+212 XXX XXX XXX";
  const storeEmail = storeSettings?.email || "";
  const footerText = storeSettings?.invoice?.footerText || "Merci pour votre confiance";
  const warrantyNotice = storeSettings?.invoice?.warrantyNotice || "La garantie est valable uniquement sur présentation de la facture.";
