...
export const SaleSchema = z.object({
  ...
  // TVA System: TVA rate (0 = no TVA, > 0 = TVA enabled)
  tvaRate: z
    .number({
      invalid_type_error: "Le taux de TVA doit être un nombre.",
    })
    .min(0, "Le taux de TVA ne peut pas être négatif.")
    .max(1, "Le taux de TVA ne peut pas dépasser 100% (1.0).")
    .default(0)
    .optional(),
  // Document System: Document creation decision
  saleDocumentType: z
    .enum(["NONE", "RECEIPT", "INVOICE"], {
      errorMap: () => ({
        message: "saleDocumentType doit être 'NONE', 'RECEIPT' ou 'INVOICE'.",
      }),
    })
    .default("NONE")
    .optional(),
