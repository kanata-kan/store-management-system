...
// In development mode with SKIP_AUTH, allows "dev-user-id" as valid ID
const objectIdSchema = z
  .string()
  .min(1, "L'identifiant est requis.")
  .refine(
    (val) => {
      // Allow dev-user-id in development mode
      if (process.env.SKIP_AUTH === "true" && val === "dev-user-id") {
        return true;
      }
      // Otherwise require valid MongoDB ObjectId format
...
// Note: managerId is optional in request body because it's added automatically
// from authenticated user in the API route
export const InventoryEntrySchema = z.object({
  ...
  managerId: objectIdSchema.optional(), // Optional in request, added by API route
  note: z
    .string()
