...
export function formatValidationError(error) {
  ...
  if (!zodError) {
    ...
    // error.message is always a string (Error instances guarantee this)
  ...
  // Handle case where issues might be empty or malformed
  ...
  const details = zodError.issues.map((issue) => {
    // Handle path: can be empty array, ["global"], or field path
    const fieldPath = issue?.path || [];
    const field = fieldPath.length > 0 ? fieldPath.join(".") : "global";
    
    // issue.message is always a string (Zod guarantee)
    let message = "";
    ...
    switch (issue?.code) {
      ...
      case "invalid_email":
        ...
        break;
      case "custom":
        // For custom errors (including refine errors), use the message directly
        // issue.message is always a string (Zod guarantee)
        const customMessage = issue?.message || "Erreur de validation";
        
        if (field === "global" || fieldPath.length === 0) {
          message = customMessage;
        } else {
          message = fieldMessages.custom(field, customMessage);
        }
        break;
      default:
        ...
        if (issue?.code === "invalid_type" && issue?.received === "undefined") {
          ...
          // For refine errors or other custom errors, use message directly
          // issue.message is always a string (Zod guarantee)
          const defaultMessage = issue?.message || "Erreur de validation";
          
          if (field === "global" || fieldPath.length === 0) {
            message = defaultMessage;
          } else {
            message = defaultMessage || fieldMessages.custom(field, `Le champ "${getFieldLabel(field)}" est invalide.`);
          }
        }
    }

    // Ensure message is always a string (should be guaranteed by Zod, but safety check)
